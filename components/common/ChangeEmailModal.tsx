      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm relative dark:text-white dark:bg-[#1A1730] dark:border dark:border-[#2A2545] dark:placeholder-gray-500 dark:caret-purple-500 max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()} style={{ scrollbarWidth: 'none' }}>
        <div className="p-6">
          <div className="flex items-center justify-between mb-5">
            <h2 className="text-lg font-bold text-gray-900 dark:text-white flex items-center gap-2"><Icon name="📧" className="w-4 h-4 inline-block mr-1 align-text-bottom" /> Alterar E-mail</h2>
          ...
          <div className="bg-gray-50 dark:bg-[#12102A] border border-gray-100 dark:border-white/5 rounded-xl p-3 mb-4 text-xs text-gray-600 dark:text-gray-300">
            <span className="font-semibold text-gray-800 dark:text-white">E-mail atual:</span> {currentEmail}
          </div>

          {success ? (
            <div className="text-center py-6">
              <p className="text-4xl mb-2"><Icon name="✅" className="w-4 h-4 inline-block mr-1 align-text-bottom" /></p>
              <p className="font-semibold text-green-600 dark:text-green-400">E-mail alterado com sucesso!</p>
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">Seu novo e-mail de acesso é: <strong className="text-gray-800 dark:text-white">{newEmail}</strong></p>
            </div>
          ) : (
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <label className="block text-xs font-semibold text-gray-600 dark:text-gray-300 uppercase mb-1">Confirme sua Senha Atual *</label>
                <input
                  type="password"
                  value={password}
              ...
              <div>
                <label className="block text-xs font-semibold text-gray-600 dark:text-gray-300 uppercase mb-1">Novo E-mail *</label>
                <input
                  type="email"
                  value={newEmail}
              ...
              <div>
                <label className="block text-xs font-semibold text-gray-600 dark:text-gray-300 uppercase mb-1">Confirmar Novo E-mail *</label>
                <input
                  type="email"
                  value={confirmEmail}
