...
interface LoginModalProps {
  isOpen: boolean;
  onClose: () => void;
  onLoginSuccess: (credentials: Credentials) => boolean | Promise<boolean>;
...
export const LoginModal: React.FC<LoginModalProps> = ({
  ...
  const handleEmailChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setUserType(lo === ADMIN_EMAIL ? 'admin' : null);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError('');
    setIsLoading(true);
    const ok = await Promise.resolve(onLoginSuccess({ email, password }));
                ...
                <div className="space-y-4">
                  {showSimulatedEmail && matchedUser && (
                    <div className="p-4 bg-amber-500/5 border border-dashed border-amber-500/35 rounded-xl text-xs space-y-2 text-gray-300">
                      ...
                      <p className="text-center text-sm font-mono font-bold bg-white/10 py-1.5 rounded tracking-widest text-white">{matchedUser.codigo}</p>
                    </div>
                  )}
