...
export const ServicesManagementTab: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      {showForm && (
        <div className="bg-white border border-gray-200 shadow-sm rounded-xl p-6 space-y-4 dark:text-white dark:bg-[#1A1730] dark:border-[#2A2545] dark:placeholder-gray-500 dark:caret-violet-500">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input value={formName} onChange={e => setFormName(e.target.value)} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm bg-white text-gray-900 dark:bg-[#2A2545] dark:border-[#3A3555] dark:text-white focus:outline-none focus:ring-1 focus:ring-violet-500" placeholder="Ex: Triagem Documental" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Grupo do Serviço *</label>
              <select value={formGroupId} onChange={e => setFormGroupId(e.target.value)} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm bg-white text-gray-900 dark:bg-[#2A2545] dark:border-[#3A3555] dark:text-white focus:outline-none focus:ring-1 focus:ring-violet-500">
          ...
          <div className="flex gap-2 pt-4">
            <button onClick={handleSave} className="px-5 py-2 text-sm font-medium text-white bg-emerald-600 rounded-lg hover:bg-emerald-700">Salvar Serviço</button>
      ...
      {groups.length === 0 ? (
        ...
        <div className="space-y-6">
          {groups.map(group => {
            ...
            return (
              <div key={group.id} className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden dark:text-white dark:bg-[#1A1730] dark:border-[#2A2545]">
                {/* Group Header */}
                ...
                {/* Group Discounts */}
                <div className="bg-violet-50/40 dark:bg-violet-950/10 px-6 py-4 border-b border-gray-200 dark:border-[#2A2545] flex flex-col md:flex-row md:items-center justify-between gap-4">
                  <div className="space-y-2 flex-1">
                    <p className="text-[10px] font-bold text-violet-700 dark:text-violet-400 uppercase tracking-widest">Desconto Geral do Grupo para Assinantes (%)</p>
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                      {[
                        ...
                        return (
                          <div key={item.role} className="flex items-center gap-2 text-xs">
                            ...
                            <input
                              ...
                              className="w-16 border border-gray-300 dark:border-[#3A3555] rounded px-2.5 py-1 text-center bg-white dark:bg-[#2A2545] text-gray-900 dark:text-white focus:outline-none focus:ring-1 focus:ring-violet-500"
                            />
                    ...
                    <tbody className="divide-y divide-gray-100 dark:divide-white/5">
                      {groupServices.map(service => (
                        <tr key={service.id} className="last:border-0 hover:bg-gray-50/50 dark:hover:bg-white/5 transition-colors">
                          ...
                          <td className="px-6 py-4 text-right font-bold text-emerald-700 dark:text-emerald-400 whitespace-nowrap">
                          ...
                          <td className="px-6 py-4 text-center space-x-3 whitespace-nowrap">
                            ...
                            <button onClick={() => handleDelete(service.id)} className="text-rose-600 hover:text-rose-800 dark:text-rose-400 dark:hover:text-rose-300" title="Excluir"><IconTrash /></button>
                          </td>
                        </tr>
                      ))}
                    </tbody>
